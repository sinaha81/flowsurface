        ...
        self.find_trade_gap()
            .and_then(|(last_t_before_gap, first_t_after_gap)| {
                ...
                if fetch_from < fetch_to {
                    // Cap the fetch range to avoid massive requests on high timeframes
                    // 500 bars is roughly 250 days on 12h, but for trades we should be more conservative.
                    // Let's cap at 500 units of the current interval.
                    let max_ms = self.interval.to_milliseconds() * 500;
                    let capped_from = fetch_to.saturating_sub(max_ms).max(fetch_from);

                    Some((capped_from, fetch_to))
                } else {
                    None
                }
            })
