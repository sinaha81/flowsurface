                ...
                if let Some(dashboard) = self.layout_manager.mut_dashboard(layout_id) {
                    ...
                    let additional_task = match event {
                        ...
                        Some(dashboard::Event::ResolveStreams { pane_id, streams }) => {
                            ...
                            let all_tickers_available = streams.iter().all(|persist| {
                                let ticker = persist.ticker();
                                tickers_info.get(ticker).and_then(|opt| *opt).is_some()
                            });

                            if !all_tickers_available {
                                // Defer resolution until all ticker info is available
                                return Task::perform(
                                    async { tokio::time::sleep(std::time::Duration::from_millis(500)).await },
                                    move |_| Message::Dashboard {
                                        layout_id: Some(layout_id),
                                        event: dashboard::Message::ResolveStreams(pane_id, streams),
                                    },
                                );
                            }

                            let resolved_streams =
                                streams.into_iter().fold(vec![], |mut acc, persist| {
