...
use data::chart::{
    Basis, ViewConfig,
    heatmap::{
        CLEANUP_THRESHOLD, Config, HeatmapDataPoint, HeatmapStudy, HistoricalDepth, QtyScale,
    },
...
const MIN_SCALING: f32 = 0.01;
const MAX_SCALING: f32 = 1000.0;

const MAX_CELL_WIDTH: f32 = 500.0;
const MIN_CELL_WIDTH: f32 = 0.1;

const MAX_CELL_HEIGHT: f32 = 500.0;
...
impl Chart for HeatmapChart {
    ...
    fn view_indicators(&'_ self, _indicators: &[Self::IndicatorKind]) -> Vec<Element<'_, Message>> {
        ...
        let bounds_size = chart_state.bounds.size();
        let region = chart_state.visible_region(bounds_size);
...
impl HeatmapChart {
    ...
    pub fn klines(&self) -> Vec<Kline> {
        self.trades
            .datapoints
            .values()
            .filter_map(|dp| DataPoint::kline(dp).cloned())
            .collect()
    }
    ...
    pub fn detached_indicator_count(&self) -> usize {
        self.modular_indicators.iter().filter(|i| !i.is_overlay()).count()
    }
                ...
                if !max_qty.is_infinite() {
                    self.heatmap
                        ...
                        .for_each(|(price, run)| {
                            ...
                            let qty_ratio = run.qty() / max_qty;
                            let bar_width = qty_ratio * 60.0;  // Increased from 50.0 for better visibility
                            
                            // Use gamma correction for sharper visual definition (Bookmap style)
                            let gamma = 0.7;  // < 1.0 makes larger orders more prominent
                            let visual_alpha = qty_ratio.powf(gamma) * 0.85;  // Boosted alpha

                            frame.fill_rectangle(
                                Point::new(0.0, y_position - (cell_height / 2.0)),
                                Size::new(bar_width, cell_height),
                                depth_color(palette, run.is_bid, visual_alpha),
                            );
                        });
...
/// Bookmap-style depth coloring with multi-color gradient
/// Uses blue → cyan → yellow → orange → red gradient for better liquidity visualization
/// Bookmap-style depth coloring with multi-color gradient
/// Uses Blue -> Cyan -> Yellow -> Orange -> Red -> White gradient based on liquidity density
fn depth_color(_palette: &Extended, _is_bid: bool, alpha: f32) -> Color {
    // Bookmap-style: Heatmap is primarily about density, not side (since Bids are below and Asks above naturally)
    
    // Non-linear intensity for better visibility of lower liquidity
    let intensity = alpha.powf(0.5).clamp(0.0, 1.0);
